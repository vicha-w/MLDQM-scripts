...
VARIATION_NAME = f"{USE_HISTOGRAM}_resnet_v4"
...
for i in range(1, 46):
    ...
    cache_isGoodLumi, cache_zphi, cache_undead_count_zphi, cache_runnr, cache_lumisection = cache.arrays(["isGoodLumi", f"hist_{USE_HISTOGRAM}", f"undead_count_{USE_HISTOGRAM}", "runnr", "lumisection"], outputtype=tuple)
    ...
    #cache_zphi = cache_zphi[(cache_isGoodLumi == 1) & (cache_undead_count_zphi >= 20000)]

    cache_zphi_refrun = cache_zphi[(cache_isGoodLumi == 1) & (~np.isnan(cache_rate)) & (cache_rate >= 75) & (cache_undead_count_zphi >= 20000)]
    cache_zphi_refrun_rate = cache_rate[(cache_isGoodLumi == 1) & (~np.isnan(cache_rate)) & (cache_rate >= 75) & (cache_undead_count_zphi >= 20000)]
    if i == 1: 
    ...
    if i == 1:
        ...
        Bchain_zphi_all_undeadcount = np.copy(cache_undead_count_zphi)
    else:
        ...
        Bchain_zphi_all_undeadcount = np.concatenate((Bchain_zphi_all_undeadcount, cache_undead_count_zphi), axis=0)
