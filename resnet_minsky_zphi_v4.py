...
VARIATION_NAME = "zphi1_resnet_v4"
...
Bchain_zphi1_refrun = []
Bchain_zphi1_refrun_rate = []

Bchain_zphi1_all = []
Bchain_zphi1_all_isgoodlumi = []
Bchain_zphi1_all_runnr = []
Bchain_zphi1_all_lumisection = []
Bchain_zphi1_all_fileloc = []
Bchain_zphi1_all_rate = []
...
for i in range(1, 46):
    ...
    if i == 1: 
        Bchain_zphi1_refrun = np.copy(cache_zphi1_refrun)
        Bchain_zphi1_refrun_rate = np.copy(cache_zphi1_refrun_rate)
    else: 
        Bchain_zphi1_refrun = np.concatenate((Bchain_zphi1_refrun, cache_zphi1_refrun), axis=0)
        Bchain_zphi1_refrun_rate = np.concatenate((Bchain_zphi1_refrun_rate, cache_zphi1_refrun_rate), axis=0)
        
    if i == 1:
        Bchain_zphi1_all = np.copy(cache_zphi1)
        Bchain_zphi1_all_isgoodlumi = np.copy(cache_isGoodLumi)
        Bchain_zphi1_all_runnr = np.copy(cache_runnr)
        Bchain_zphi1_all_lumisection = np.copy(cache_lumisection)
        Bchain_zphi1_all_fileloc = np.array([i]*len(cache_runnr))
        Bchain_zphi1_all_rate = np.copy(cache_rate)
        ...
        Bchain_zphi1_all_isgoodlumi = np.concatenate((Bchain_zphi1_all_isgoodlumi, cache_isGoodLumi), axis=0)
        Bchain_zphi1_all_runnr = np.concatenate((Bchain_zphi1_all_runnr, cache_runnr), axis=0)
        Bchain_zphi1_all_lumisection = np.concatenate((Bchain_zphi1_all_lumisection, cache_lumisection), axis=0)
        Bchain_zphi1_all_fileloc = np.concatenate((Bchain_zphi1_all_fileloc, np.array([i]*len(cache_runnr))), axis=0)
        Bchain_zphi1_all_rate = np.concatenate((Bchain_zphi1_all_rate, cache_rate), axis=0)
...
Bchain_zphi1_2D = np.asarray([hist/rate for hist, rate in zip(Bchain_zphi1_2D, Bchain_zphi1_refrun_rate)])
...
Bchain_zphi1_2D_all = np.reshape(Bchain_zphi1_all, (-1, 202, 302))[:, 1:201, 80:220]
Bchain_zphi1_2D_all = np.reshape(Bchain_zphi1_2D_all, (-1, 200*140))
Bchain_zphi1_2D_all = np.asarray([hist/rate for hist, rate in zip(Bchain_zphi1_2D_all, Bchain_zphi1_all_rate)])
Bchain_zphi1_2D_all = np.reshape(Bchain_zphi1_2D_all, (-1, 200, 140, 1))
...
callbacks = [
    ...
    #keras.callbacks.ModelCheckpoint(filepath="{1}/resnet_minsky_zphi1_{0}_{{epoch:03d}}.hdf5".format(variant_suffix, VARIATION_NAME)),
    keras.callbacks.ModelCheckpoint(filepath="{1}/resnet_minsky_zphi1_{0}_best.hdf5".format(variant_suffix, VARIATION_NAME), save_best_only=True)
]
history = resnet_model.fit(Bchain_zphi1_2D_train, Bchain_zphi1_2D_train, epochs=TRAINING_EPOCHS, verbose=True, validation_data=(Bchain_zphi1_2D_test, Bchain_zphi1_2D_test), batch_size=100, callbacks=callbacks)
#resnet_model.save("{1}/resnet_minsky_zphi1_{0}.hdf5".format(variant_suffix, VARIATION_NAME))
with open("{1}/resnet_minsky_zphi1_{0}_history.p".format(variant_suffix, VARIATION_NAME), "wb") as history_pickle:
    pickle.dump(history.history, history_pickle)

resnet_model = keras.models.load_model("{1}/resnet_minsky_zphi1_{0}_best.hdf5".format(variant_suffix, VARIATION_NAME))
...
all_predictions_cache = resnet_model.predict(Bchain_zphi1_2D_all)
resnet_all_losses = keras.losses.mse(Bchain_zphi1_2D_all, all_predictions_cache)
...
resnet_alllumi_df["runnr"] = Bchain_zphi1_all_runnr
resnet_alllumi_df["lumisection"] = Bchain_zphi1_all_lumisection
resnet_alllumi_df["fileloc"] = Bchain_zphi1_all_fileloc
resnet_alllumi_df["isGoodLumi"] = Bchain_zphi1_all_isgoodlumi
resnet_alllumi_df["rate"] = Bchain_zphi1_all_rate
resnet_alllumi_df["undead_count"] = Bchain_zphi1_all_undeadcount
